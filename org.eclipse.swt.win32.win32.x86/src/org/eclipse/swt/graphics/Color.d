...
public final class Color : Resource {
    ...
    package alias init_ = Resource.init_;
 ...
 * @since 3.104
 */
public this (Device device, int red, int green, int blue, int alpha) {
    super(device);
    init_(red, green, blue, alpha);
    init_();
}
...
public this (Device device, RGB rgb) {
    ...
    init_(rgb.red, rgb.green, rgb.blue, 255);
    init_();
}
 ...
 * @since 3.104
 */
public this (Device device, RGBA rgba) {
    super(device);
    if (rgba is null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
    init_(rgba.rgb.red, rgba.rgb.green, rgba.rgb.blue, rgba.alpha);
    init_();
}
 ...
 * @since 3.104
 */
public this (Device device, RGB rgb, int alpha) {
    super(device);
    if (rgb is null) SWT.error(SWT.ERROR_NULL_ARGUMENT);
    init_(rgb.red, rgb.green, rgb.blue, alpha);
    init_();
}
 ...
 */
override
public bool opEquals (Object object) const {
    if (object is this) return true;
    if (!(cast(Color)object)) return false;
    Color color = cast(Color) object;
    return device is color.device && (handle & 0xFFFFFF) is (color.handle & 0xFFFFFF) && (alpha == color.alpha);
}
 ...
 * @see #equals
 */
deprecated("Use org.eclipse.swt.graphics.Color.toHash instead.")
public int hashCode () {
    return handle ^ alpha;
}
 ...
 * @see #dispose
 */
package void init_(int red, int green, int blue, int alpha) {
...
override public String toString () const {
    ...
    return Format( "Color {{{}, {}, {}, {}}", getRed(), getGreen(), getBlue(), getAlpha()); //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
}
